...
OBJECT_ID='SL-VAL-TREND-PULLBACK-SIDE-ASYMMETRY-METRIC-CORRECTED-EXEC-001'
SYMBOL='ETHUSDT'
...
def metric_invariants():
 cases=[(100.0,110.0),(100.0,90.0),(250.0,250.0),(80.0,100.0)]
 for entry,exitp in cases:
  q=exitp/entry
  long_ret=q-1
  short_ret=1-q
  if abs(long_ret+short_ret)>1e-15: return False,'long-short-antisymmetry'
  if abs(short_ret-(entry-exitp)/entry)>1e-15: return False,'short-entry-notional-equivalence'
  if entry==exitp and (long_ret!=0 or short_ret!=0): return False,'flat-return-zero'
 return True,None
...
def main(out,job):
 try:
  ...
  emit(out,'EVIDENCE-FAIL',reason='job-contract:'+repr(e),parameter_tuning=False);return
 ok,reason=metric_invariants()
 if not ok:
  emit(out,'EVIDENCE-FAIL',reason='metric-invariant:'+str(reason),parameter_tuning=False);return
 try:
  rows=[];sources=[]
 ...
 for i in range(begin,len(rows)-HOLD):
  ...
  entry=rows[i+1]['o'];exitp=rows[i+HOLD]['o'];q=exitp/entry
 ...
 if l10['n']<MIN_SIDE_N or s10['n']<MIN_SIDE_N:
  ...
  state='METRIC-CORRECTED-ASYMMETRY-SUPPORTED' if supported else 'METRIC-CORRECTED-ASYMMETRY-NOT-SUPPORTED'
 emit(out,state,
      market=f'{SYMBOL} Binance USDT-M perpetual',
      contract={'parent_object':'SL-VAL-TREND-PULLBACK-001','predecessor':'SL-VAL-TREND-PULLBACK-SIDE-ASYMMETRY-001','purpose':'falsify prior side-asymmetry result after correcting asymmetric SHORT return metric','tf':'4h','trend':'strictly-prior 180-bar return sign','pullback':'3-bar counter-trend move whose absolute return exceeds strictly-prior 1080-bar 90th percentile','entry':'next_bar_open','hold_bars':12,'decluster_bars':18,'split':'2025-01-01','cost_10bps':COST,'cost_stress_20bps':COST_STRESS,'side_min_n':MIN_SIDE_N,'material_gap':MATERIAL_GAP,'short_return':'1-exit_over_entry on fixed entry-notional basis','long_return':'exit_over_entry-1','metric_invariants_passed':True,'parameter_tuning':False},
      ...
      limitations=['USDT-M perpetual remains correlated with Spot and is not independent macro-regime evidence','fixed 10/20bp round-trip drag does not model funding, dynamic fees, spread, slippage, latency or impact','this test changes only the SHORT return measurement basis; no signal, split, hold, decluster, cost, side threshold or materiality parameter is changed','support would validate directional asymmetry only, not promote the parent strategy'])
