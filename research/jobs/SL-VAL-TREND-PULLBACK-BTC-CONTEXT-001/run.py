...
OBJECT_ID='SL-VAL-TREND-PULLBACK-BTC-CONTEXT-001'
SYMBOL='ETHUSDT'
BTC='BTCUSDT'
BASE=f'https://data.binance.vision/data/spot/monthly/klines/{SYMBOL}/4h'
BTC_BASE=f'https://data.binance.vision/data/spot/monthly/klines/{BTC}/4h'
...
def mon(y,m,symbol=SYMBOL,base=BASE):
...
def main(out,job):
 try:
  ...
  for y in range(2020,2027):
   for m in range(1,13):
    ...
    ba,bb=mon(y,m,BTC,BTC_BASE);btc_rows+=ba;files.append(bb)
 except Exception as e: emit(out,'PROVIDER-BLOCKED',reason=repr(e),parameter_tuning=False);return
 rows.sort(key=lambda x:x['ts']);btc_rows.sort(key=lambda x:x['ts'])
 btc_by_ts={x['ts']:x for x in btc_rows};btc_idx={x['ts']:i for i,x in enumerate(btc_rows)}
 events=[];last=-10**9
 for i in range(max(TREND,PULL,ROLL)+1,len(rows)-HOLD):
  ...
  bi=btc_idx.get(rows[i-1]['ts'])
  if bi is None or bi<180:continue
  btc_ret=math.log(btc_rows[bi]['c']/btc_rows[bi-180]['c'])
  btc_direction='UP' if btc_ret>0 else 'DOWN' if btc_ret<0 else 'NEUTRAL'
  alignment='WITH-BTC' if (side=='LONG' and btc_direction=='UP') or (side=='SHORT' and btc_direction=='DOWN') else 'COUNTER-BTC'
  events.append({'ts':b['ts'],'side':side,'btc_direction':btc_direction,'alignment':alignment,'gross':gross,'net':gross-COST,'net20':gross-.002,'period':'IS' if b['ts']<int(SPLIT.timestamp()) else 'OOS'})
  last=i
 ...
 wb=st([x['net'] for x in o if x['alignment']=='WITH-BTC'])
 cb=st([x['net'] for x in o if x['alignment']=='COUNTER-BTC'])
 if wb['n']<10 or cb['n']<10:v='BLOCKED-EVIDENCE'
 elif abs((wb['mean'] or 0)-(cb['mean'] or 0))>=.0025:v='BTC-CONTEXT-DEPENDENCE-SUPPORTED'
 else:v='BTC-CONTEXT-DEPENDENCE-NOT-SUPPORTED'
 emit(out,'OUTCOME-COMPLETE',market=f'{SYMBOL} Spot',
      contract={'parent_object':'SL-VAL-TREND-PULLBACK-001','purpose':'outcome-blind frozen BTC cross-market context falsifier','market':f'{SYMBOL} Spot','tf':'4h','trend':'strictly-prior ETH 180-bar return sign','btc_context':'strictly-prior BTC 180-bar return sign at same timestamp','pullback':'3-bar counter-trend move whose absolute return exceeds strictly-prior 1080-bar 90th percentile','entry':'next_bar_open','hold_bars':12,'decluster_bars':18,'cost':.001,'cost_stress':.002,'split':'2025-01-01','no_sl_tp':True,'parameter_tuning':False},
      ...
      oos_with_btc=wb,oos_counter_btc=cb,
