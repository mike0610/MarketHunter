...
def fetch_raw(path, params=None, attempts=FAST_ATTEMPTS):
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'WEEX-spot-radar/0.14.1-background-mtf-paper'})
    last_exc = None
...
def analyze(conn, symbol, m15=None):
    ...
    return {'symbol': symbol, 'state': state, 'direction': direction,
            ...
            'rules_version': 'experimental_v0_14_background_mtf_long_short_1d_1h_15m'}
...
def cycle(conn, limit):
    ...
    METRICS = {'api_calls': {}, 'api_seconds': {}, 'retries': 0, 'slow_calls': 0,
               'cache_1d_hit': 0, 'cache_1d_miss': 0, 'cache_1h_hit': 0, 'cache_1h_miss': 0, 'negative_1d_hit': 0, 'negative_1h_hit': 0, 'cache_pending': 0, 'refresh_queue': 0}
    symbols = universe()
    tickers = snapshot()
