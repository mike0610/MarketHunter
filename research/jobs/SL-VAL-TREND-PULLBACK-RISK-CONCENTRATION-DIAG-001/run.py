...
OBJECT_ID='SL-VAL-TREND-PULLBACK-RISK-CONCENTRATION-DIAG-001'
SYMBOL='ETHUSDT'
...
MID=datetime(2025,10,16,12,0,tzinfo=timezone.utc)
...
def top_tail(events):
 ordered=sorted(events,key=lambda x:x['net_20bp'],reverse=True);k=max(1,math.ceil(len(ordered)*.10));top=ordered[:k]
 pos=sum(max(0,x['net_20bp']) for x in ordered)
 return {'k':k,'sum':sum(x['net_20bp'] for x in top),'share_of_positive_gains':sum(max(0,x['net_20bp']) for x in top)/pos if pos>0 else None,
  'side_counts':{s:sum(x['side']==s for x in top) for s in ['LONG','SHORT']},
  'half_counts':{h:sum(x['half']==h for x in top) for h in ['OOS_A','OOS_B']},
  'trades':[{'signal_ts':x['signal_ts'],'signal_iso':datetime.fromtimestamp(x['signal_ts'],timezone.utc).isoformat(),'side':x['side'],'half':x['half'],'net_20bp':x['net_20bp'],'gross':x['gross'],'funding_return':x['funding_return']} for x in top]}

def main(out,job):
 try:
  cfg=json.loads(Path(job).read_text())
  expected={'CONCENTRATION-SIDE-AND-TEMPORAL','CONCENTRATION-SIDE-LOCALIZED','CONCENTRATION-TEMPORAL-LOCALIZED','CONCENTRATION-NONLOCALIZED','PREDECESSOR-NOT-REPRODUCED','INSUFFICIENT-OOS-BREADTH','EVIDENCE-FAIL','PROVIDER-BLOCKED'}
 ...
 for i in range(begin,len(rows)-HOLD):
  ...
  events.append({'signal_ts':b['ts'],'side':side,'net_20bp':net,'gross':gross,'funding_return':funding_return,'period':'IS' if b['ts']<int(SPLIT.timestamp()) else 'OOS','half':'OOS_A' if b['ts']<int(MID.timestamp()) else 'OOS_B'})
  last=i
 ...
 full=stats([x['net_20bp'] for x in oos]);ordered=sorted(oos,key=lambda x:x['net_20bp'],reverse=True);k=max(1,math.ceil(len(oos)*.10))
 ex1=stats([x['net_20bp'] for x in ordered[1:]])
 ex10=stats([x['net_20bp'] for x in ordered[k:]])
 if full['mean'] is None or full['mean']<=0 or (ex1['mean'] is not None and ex1['mean']>0 and ex10['mean'] is not None and ex10['mean']>0):
  emit(out,'PREDECESSOR-NOT-REPRODUCED',baseline=full,mean_ex_best_1=ex1['mean'],mean_ex_best_10pct=ex10['mean'],parameter_tuning=False);return
 side={s:stats([x['net_20bp'] for x in oos if x['side']==s]) for s in ['LONG','SHORT']}
 half={h:stats([x['net_20bp'] for x in oos if x['half']==h]) for h in ['OOS_A','OOS_B']}
 side_pos=[s for s in side if side[s]['mean'] is not None and side[s]['mean']>0]
 half_pos=[h for h in half if half[h]['mean'] is not None and half[h]['mean']>0]
 side_local=len(side_pos)==1
 temporal_local=len(half_pos)==1
 if side_local and temporal_local: state='CONCENTRATION-SIDE-AND-TEMPORAL'
 elif side_local: state='CONCENTRATION-SIDE-LOCALIZED'
 elif temporal_local: state='CONCENTRATION-TEMPORAL-LOCALIZED'
 else: state='CONCENTRATION-NONLOCALIZED'
 emit(out,state,
  market=f'{SYMBOL} Binance USDT-M perpetual',
  contract={'predecessor':'SL-VAL-TREND-PULLBACK-RISK-BEHAVIOR-001','signal_entry_exit_funding':'identical to predecessor','classification_basis':'actual funding plus frozen 20bp all-in friction','oos_midpoint':MID.isoformat(),'localization_rule':'sign-only, no tuned magnitude threshold: localized iff exactly one subgroup mean > 0 and the other <= 0','parameter_tuning':False},
  predecessor_reproduction={'baseline':full,'mean_ex_best_1':ex1['mean'],'mean_ex_best_10pct':ex10['mean'],'top_10pct_k':k},
  side_stats=side,half_stats=half,top_tail=top_tail(oos),source_files=sources,parameter_tuning=False,
  limitations=['localization is descriptive sign structure, not a causal regime model','top-winner attribution does not prove why the winners occurred','historical spread, realized slippage, latency, queue position, partial fills, market impact and venue outages remain outside the reconstruction','no side disable, regime filter, sizing, leverage, allocation or canonical promotion follows from this diagnostic','if concentration is nonlocalized, this branch should not be rescued by finer partition mining without a separate material hypothesis'])
