...
OBJECT_ID='SL-VAL-TREND-PULLBACK-AGGFLOW-PORTABILITY-001'
...
def mon(symbol,y,m):
 n=f'{symbol}-4h-{y}-{m:02d}.zip';u=f'https://data.binance.vision/data/spot/monthly/klines/{symbol}/4h/{n}'
...
def evaluate(symbol):
 ...
 flow={}
 def flowmon(y,m):
  n=f'{symbol}-4h-{y}-{m:02d}.zip';u=f'https://data.binance.vision/data/futures/um/monthly/klines/{symbol}/4h/{n}'
  z=get(u);e=get(u+'.CHECKSUM').decode().split()[0].lower();a=hashlib.sha256(z).hexdigest()
  if e!=a: raise ValueError('checksum '+n)
  with zipfile.ZipFile(io.BytesIO(z)) as q:
   for x in csv.reader(io.TextIOWrapper(q.open([v for v in q.namelist() if not v.endswith('/')][0]))):
 ...
 return {'symbol':symbol,'is_stats':st([x['net'] for x in events if x['period']=='IS']),'oos_stats_10bps':s,'oos_stats_20bps':s20,'oos_long':st([x['net'] for x in o if x['side']=='LONG']),'oos_short':st([x['net'] for x in o if x['side']=='SHORT']),'oos_with_flow':wf,'oos_counter_flow':cf,'oos_top_positive_trade_share':top,'terminal_verdict':v,'event_count':len(events),'source_files':files}

def main(out,job):
 try:
  if json.loads(Path(job).read_text()).get('object_id')!=OBJECT_ID:raise ValueError('object')
  results=[evaluate(s) for s in ['BTCUSDT','ETHUSDT','SOLUSDT']]
 except Exception as e:
  emit(out,'PROVIDER-BLOCKED',reason=repr(e),parameter_tuning=False);return
 supported=[r for r in results if r['terminal_verdict']=='AGGFLOW-CONTEXT-SUPPORTED']
 comparable=[r for r in results if r['terminal_verdict']!='BLOCKED-EVIDENCE']
 verdict='PORTABLE-SUPPORTED' if len(supported)>=2 and len(comparable)>=2 else ('PORTABLE-NOT-SUPPORTED' if len(comparable)>=2 else 'BLOCKED-EVIDENCE')
 emit(out,'OUTCOME-COMPLETE',contract={'symbols':['BTCUSDT','ETHUSDT','SOLUSDT'],'tf':'4h','parameters':'identical to parent frozen object','split':'2025-01-01','parameter_tuning':False},per_symbol=results,terminal_verdict=verdict,parameter_tuning=False,limitations=['three large Binance-listed crypto assets','same fixed parameters across assets','requires later regime and execution robustness if supported'])
