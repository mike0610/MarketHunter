...
OBJECT_ID='SL-VAL-INTRADAY-REVERSAL-001'
...
def main(out,job):
 ...
 for i in range(ROLL+1,len(rows)-HOLD):
  ...
  if i-last_event<DECLUSTER:continue
  ...
  side='SHORT' if r>0 else 'LONG';entry=rows[i+1]['o'];exitp=rows[i+HOLD]['o'];gross=exitp/entry-1 if side=='LONG' else entry/exitp-1
 ...
 emit(out,'OUTCOME-COMPLETE',contract={'market':'BTCUSDT Spot','tf':'1h','shock':'abs close-to-close log return > strictly-prior 720h 95th percentile','direction':'fade shock','hold_bars':3,'decluster_bars':6,'entry':'next_bar_open','cost':.001,'cost_stress':.002,'split':'2025-01-01','no_sl_tp':True,'parameter_tuning':False},is_stats=st([x['net'] for x in events if x['period']=='IS']),oos_stats_10bps=s,oos_stats_20bps=s20,oos_long=st([x['net'] for x in o if x['side']=='LONG']),oos_short=st([x['net'] for x in o if x['side']=='SHORT']),oos_top_positive_trade_share=top_share,terminal_verdict=v,event_count=len(events),source_files=files,parameter_tuning=False,limitations=['single asset first-pass','no spread/slippage beyond fixed round-trip cost','no funding or leverage claim','candidate requires separate robustness gate'])
