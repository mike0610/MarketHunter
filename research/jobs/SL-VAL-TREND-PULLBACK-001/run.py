...
OBJECT_ID='SL-VAL-TREND-PULLBACK-001'
...
def main(out,job):
 ...
 for i in range(max(TREND,PULL,ROLL)+1,len(rows)-HOLD):
  ...
  events.append({'ts':b['ts'],'side':side,'trend_ret':trend_ret,'pull_ret':pull_ret,'threshold':th,'gross':gross,'net':gross-COST,'net20':gross-.002,'period':'IS' if b['ts']<int(SPLIT.timestamp()) else 'OOS'})
  last=i
 ...
 emit(out,'OUTCOME-COMPLETE',
      ...
      limitations=['single asset first-pass','fixed hold without stop/target','no spread/slippage beyond fixed round-trip cost','candidate requires separate robustness and capital-survival gate'])
