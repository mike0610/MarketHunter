...
OBJECT_ID='SL-VAL-TREND-PULLBACK-RISK-BEHAVIOR-001'
SYMBOL='ETHUSDT'
...
def tail_diagnostics(v):
 n=len(v);ordered=sorted(v,reverse=True)
 k=max(1,math.ceil(n*.10))
 full=sum(v)
 positive=sum(x for x in v if x>0)
 top1=ordered[0]
 topk=ordered[:k]
 drop1=v.copy();drop1.remove(top1)
 dropk=ordered[k:]
 worst_k=sorted(v)[:k]
 return {
  'best_1_return':top1,
  'best_1_share_of_positive_gains':top1/positive if positive>0 else None,
  'top_10pct_k':k,
  'top_10pct_sum':sum(topk),
  'top_10pct_share_of_positive_gains':sum(x for x in topk if x>0)/positive if positive>0 else None,
  'mean_ex_best_1':sum(drop1)/len(drop1) if drop1 else None,
  'mean_ex_best_10pct':sum(dropk)/len(dropk) if dropk else None,
  'worst_10pct_mean':sum(worst_k)/len(worst_k),
  'longest_losing_streak':losing_streak(v),
  'full_sum':full
 }
...
def main(out,job):
 try:
  ...
  expected={'RISK-BEHAVIOR-BROAD','RISK-BEHAVIOR-CONCENTRATED','INSUFFICIENT-OOS-BREADTH','EVIDENCE-FAIL','PROVIDER-BLOCKED'}
 ...
 for i in range(begin,len(rows)-HOLD):
  ...
  events.append({'signal_ts':b['ts'],'side':side,'net_20bp':net,'period':'IS' if b['ts']<int(SPLIT.timestamp()) else 'OOS'})
  last=i
 ...
 v=[x['net_20bp'] for x in oos]
 base=stats(v)
 if base['mean'] is None or base['mean']<=0:
  emit(out,'EVIDENCE-FAIL',reason='predecessor-execution-margin-did-not-reproduce',oos_20bp=base,parameter_tuning=False);return
 diag=tail_diagnostics(v)
 broad=diag['mean_ex_best_1']>0 and diag['mean_ex_best_10pct']>0
 state='RISK-BEHAVIOR-BROAD' if broad else 'RISK-BEHAVIOR-CONCENTRATED'
 emit(out,state,
  market=f'{SYMBOL} Binance USDT-M perpetual',
  contract={'predecessor':'SL-VAL-TREND-PULLBACK-EXECUTION-REALISM-001','signal_entry_exit_funding':'identical to predecessor','classification_basis':'actual funding plus frozen 20bp all-in friction','oos_min_n':OOS_MIN_N,'anti_concentration_stress':['remove single best OOS trade','remove best ceil(10% of OOS trades)'],'broad_gate':'baseline mean > 0 AND mean excluding best single trade > 0 AND mean excluding best ceil(10%) trades > 0','drawdown_and_tail_metrics':'diagnostic only; no sizing or leverage is inferred','parameter_tuning':False},
  oos_20bp=base,
  concentration=diag,
  side_stats={'LONG':stats([x['net_20bp'] for x in oos if x['side']=='LONG']),'SHORT':stats([x['net_20bp'] for x in oos if x['side']=='SHORT'])},
  event_count=len(events),oos_event_count=len(oos),source_files=sources,parameter_tuning=False,
  limitations=['the anti-concentration stress tests robustness of expectancy to extreme winners; it is not a portfolio sizing rule','max drawdown is a sequential unit-notional diagnostic, not a real portfolio drawdown because capital allocation and concurrent exposure are not modeled','historical spread, realized slippage, latency, queue position, partial fills, market impact and venue outages remain outside the 4h reconstruction','a concentrated result does not prove the parent edge is false; it materially downgrades risk robustness and requires no rescue tuning','no LONG-disable, SHORT-only rule, leverage, sizing, allocation or canonical promotion follows from this object'])
