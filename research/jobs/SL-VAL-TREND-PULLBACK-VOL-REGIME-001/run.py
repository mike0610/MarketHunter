...
OBJECT_ID='SL-VAL-TREND-PULLBACK-VOL-REGIME-001'
SYMBOL='ETHUSDT'
...
def main(out,job):
 ...
 for i in range(max(TREND,PULL,ROLL)+1,len(rows)-HOLD):
  ...
  for j in range(i-168,i):
   if j<=0:continue
   rv.append(abs(math.log(rows[j]['c']/rows[j-1]['c'])))
  rv_hist=[]
  for j in range(i-1080,i):
   if j<=0:continue
   rv_hist.append(abs(math.log(rows[j]['c']/rows[j-1]['c'])))
  vol=sum(rv)/len(rv)
  vol_med=quantile(rv_hist,.50)
  vol_regime='HIGH' if vol>vol_med else 'LOW'
  events.append({'ts':b['ts'],'side':side,'vol_regime':vol_regime,'gross':gross,'net':gross-COST,'net20':gross-.002,'period':'IS' if b['ts']<int(SPLIT.timestamp()) else 'OOS'})
  last=i
 ...
 hi=st([x['net'] for x in o if x['vol_regime']=='HIGH'])
 lo=st([x['net'] for x in o if x['vol_regime']=='LOW'])
 if hi['n']<10 or lo['n']<10:v='BLOCKED-EVIDENCE'
 elif abs((hi['mean'] or 0)-(lo['mean'] or 0))>=.0025:v='VOL-REGIME-DEPENDENCE-SUPPORTED'
 else:v='VOL-REGIME-DEPENDENCE-NOT-SUPPORTED'
 emit(out,'OUTCOME-COMPLETE',market=f'{SYMBOL} Spot',
      contract={'parent_object':'SL-VAL-TREND-PULLBACK-001','purpose':'outcome-blind frozen volatility-regime falsifier','market':f'{SYMBOL} Spot','tf':'4h','trend':'strictly-prior 180-bar return sign','vol_regime':'strictly-prior 168h mean absolute 4h return vs strictly-prior 1080-bar median','pullback':'3-bar counter-trend move whose absolute return exceeds strictly-prior 1080-bar 90th percentile','entry':'next_bar_open','hold_bars':12,'decluster_bars':18,'cost':.001,'cost_stress':.002,'split':'2025-01-01','no_sl_tp':True,'parameter_tuning':False},
      ...
      oos_high_vol=hi,oos_low_vol=lo,
      oos_top_positive_trade_share=top,terminal_verdict=v,event_count=len(events),source_files=files,parameter_tuning=False,
      limitations=['single ETHUSDT volatility-regime falsifier only','fixed hold without stop/target','no spread/slippage beyond fixed round-trip cost','support does not imply promotion eligibility'])
