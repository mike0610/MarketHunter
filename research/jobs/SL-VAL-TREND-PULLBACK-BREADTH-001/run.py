...
OBJECT_ID='SL-VAL-TREND-PULLBACK-BREADTH-001'
SYMBOL='ETHUSDT'
BASKET=['BTCUSDT','BNBUSDT','ADAUSDT','XRPUSDT','DOGEUSDT','LINKUSDT','LTCUSDT']
...
def main(out,job):
 try:
  ...
  rows=[];breadth_rows={s:[] for s in BASKET};files=[]
  for y in range(2020,2027):
   for m in range(1,13):
    ...
    for sym in BASKET:
     ba,bb=mon(y,m,sym);breadth_rows[sym]+=ba;files.append(bb)
 except Exception as e: emit(out,'PROVIDER-BLOCKED',reason=repr(e),parameter_tuning=False);return
 rows.sort(key=lambda x:x['ts'])
 breadth_maps={}
 for sym,arr in breadth_rows.items():
  arr.sort(key=lambda x:x['ts']);breadth_maps[sym]={x['ts']:x for x in arr}
 events=[];last=-10**9
 for i in range(max(TREND,PULL,ROLL)+1,len(rows)-HOLD):
  ...
  pos=neg=avail=0
  prior_ts=rows[i-1]['ts']; prior2_ts=rows[i-2]['ts']
  for sym in BASKET:
   a=breadth_maps[sym].get(prior_ts);p=breadth_maps[sym].get(prior2_ts)
   if a and p and a['c']>0 and p['c']>0:
    avail+=1
    r=math.log(a['c']/p['c'])
    pos+=r>0;neg+=r<0
  if avail<6:continue
  breadth=(pos-neg)/avail
  breadth_regime='POSITIVE' if breadth>0 else 'NEGATIVE' if breadth<0 else 'NEUTRAL'
  alignment='WITH-BREADTH' if (side=='LONG' and breadth_regime=='POSITIVE') or (side=='SHORT' and breadth_regime=='NEGATIVE') else 'COUNTER-BREADTH'
  events.append({'ts':b['ts'],'side':side,'breadth':breadth,'breadth_regime':breadth_regime,'alignment':alignment,'gross':gross,'net':gross-COST,'net20':gross-.002,'period':'IS' if b['ts']<int(SPLIT.timestamp()) else 'OOS'})
  last=i
 ...
 wb=st([x['net'] for x in o if x['alignment']=='WITH-BREADTH'])
 cb=st([x['net'] for x in o if x['alignment']=='COUNTER-BREADTH'])
 if wb['n']<10 or cb['n']<10:v='BLOCKED-EVIDENCE'
 elif abs((wb['mean'] or 0)-(cb['mean'] or 0))>=.0025:v='BREADTH-DEPENDENCE-SUPPORTED'
 else:v='BREADTH-DEPENDENCE-NOT-SUPPORTED'
 emit(out,'OUTCOME-COMPLETE',market=f'{SYMBOL} Spot',
      contract={'parent_object':'SL-VAL-TREND-PULLBACK-001','purpose':'outcome-blind frozen cross-crypto breadth falsifier','market':f'{SYMBOL} Spot','tf':'4h','trend':'strictly-prior ETH 180-bar return sign','breadth':'same-prior-bar signed breadth across fixed BTC/BNB/ADA/XRP/DOGE/LINK/LTC basket','pullback':'3-bar counter-trend move whose absolute return exceeds strictly-prior 1080-bar 90th percentile','entry':'next_bar_open','hold_bars':12,'decluster_bars':18,'cost':.001,'cost_stress':.002,'split':'2025-01-01','no_sl_tp':True,'parameter_tuning':False},
      ...
      oos_with_breadth=wb,oos_counter_breadth=cb,
