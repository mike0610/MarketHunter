...
OBJECT_ID='SL-VAL-TREND-PULLBACK-SIDE-ASYMMETRY-DEPENDENCE-001'
SYMBOL='ETHUSDT'
...
CUT=datetime(2025,10,16,12,tzinfo=timezone.utc)  # exact midpoint of frozen OOS window
TREND=180
PULL=3
ROLL=1080
Q=.90
...
PART_MIN_SIDE_N=5  # original pooled breadth gate divided across two frozen equal-duration partitions
MATERIAL_GAP=.0025
...
def main(out,job):
 try:
  ...
  expected={'DEPENDENCE-BROAD','DEPENDENCE-CONCENTRATED','INSUFFICIENT-PARTITION-BREADTH','EVIDENCE-FAIL','PROVIDER-BLOCKED'}
 ...
 if not passes(pl10,ps10,pl20,ps20,POOLED_MIN_SIDE_N):
  emit(out,'EVIDENCE-FAIL',reason='predecessor-pooled-support-not-reproduced',pooled={'long10':pl10,'short10':ps10,'long20':pl20,'short20':ps20},parameter_tuning=False);return
 if min(a_l10['n'],a_s10['n'],b_l10['n'],b_s10['n'])<PART_MIN_SIDE_N:
  state='INSUFFICIENT-PARTITION-BREADTH'
 else:
  state='DEPENDENCE-BROAD' if (passes(a_l10,a_s10,a_l20,a_s20,PART_MIN_SIDE_N) and passes(b_l10,b_s10,b_l20,b_s20,PART_MIN_SIDE_N)) else 'DEPENDENCE-CONCENTRATED'
 emit(out,state,
  market=f'{SYMBOL} Binance USDT-M perpetual',
  contract={'parent_object':'SL-VAL-TREND-PULLBACK-001','predecessor':'SL-VAL-TREND-PULLBACK-SIDE-ASYMMETRY-METRIC-CORRECTED-EXEC-001','purpose':'test temporal dependence of corrected LONG/SHORT asymmetry without side-rule tuning','tf':'4h','trend':'strictly-prior 180-bar return sign','pullback':'3-bar counter-trend move whose absolute return exceeds strictly-prior 1080-bar 90th percentile','entry':'next_bar_open','hold_bars':12,'decluster_bars':18,'split':'2025-01-01','oos_end':'2026-08-01','partition_cut':'2025-10-16T12:00:00Z exact midpoint of frozen OOS window','partition_min_side_n':PART_MIN_SIDE_N,'derivation':'pooled n>=10 breadth gate divided across two equal-duration partitions','cost_10bps':COST,'cost_stress_20bps':COST_STRESS,'material_gap':MATERIAL_GAP,'short_return':'1-exit_over_entry fixed entry-notional','long_return':'exit_over_entry-1','parameter_tuning':False},
  pooled={'long_10bps':pl10,'short_10bps':ps10,'long_20bps':pl20,'short_20bps':ps20},
  oos_a={'start':'2025-01-01T00:00:00Z','end':'2025-10-16T12:00:00Z','long_10bps':a_l10,'short_10bps':a_s10,'long_20bps':a_l20,'short_20bps':a_s20,'support_gate':passes(a_l10,a_s10,a_l20,a_s20,PART_MIN_SIDE_N)},
  oos_b={'start':'2025-10-16T12:00:00Z','end':'2026-08-01T00:00:00Z','long_10bps':b_l10,'short_10bps':b_s10,'long_20bps':b_l20,'short_20bps':b_s20,'support_gate':passes(b_l10,b_s10,b_l20,b_s20,PART_MIN_SIDE_N)},
  event_count=len(events),source_files=sources,parameter_tuning=False,
  limitations=['two temporal halves share the same ETHUSDT perpetual venue and macro regime lineage','partition test is a dependence/breadth check, not independent edge replication','fixed 10/20bp drag omits funding, dynamic spread/slippage, latency and impact','DEPENDENCE-BROAD would preserve directional-dependence concern only; it would not authorize LONG-disable or SHORT-only strategy','INSUFFICIENT-PARTITION-BREADTH blocks generalization rather than rejecting the parent strategy'])
