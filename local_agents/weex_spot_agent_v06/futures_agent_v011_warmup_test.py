"""WEEX futures-only accumulation/breakout radar. Public endpoints; PAPER ONLY, NEVER places orders."""
import argparse
import os
import sys
import traceback
...
DB = ROOT / 'futures_signals_v011_warmup_test.sqlite3'
CONFIG = ROOT / 'config.json'
LOG = ROOT / 'futures_agent_v011_warmup_test.log'
PID = ROOT / 'futures_agent_v011_warmup_test.pid'
...
def telegram(event):
    # Isolated v0.1.1 warm-up test: never sends Telegram messages.
    return False


def _telegram_disabled_original(event):
    ...
    try:
        ...
        if state == 'BREAKOUT_CANDIDATE':
            ...
            msg = ('🚨 WEEX FUTURES AGENT SYSTEM ERROR\n'
                   ...
                   'Агент залишається PAPER ONLY. Перевір журнал futures_agent_v011_warmup_test.log.')
...
def analyze(conn, symbol, m15=None):
    ...
    return {'symbol': symbol, 'state': state, 'direction': direction,
            ...
            'rules_version': 'futures_v0_1_1_warmup_test_long_short_1d_1h_15m'}
...
def main():
    ...
    if args.warmup_seconds < 30 or args.warmup_seconds > 1800:
        parser.error('--warmup-seconds must be 30..1800')
    if PID.exists():
        parser.error('futures_agent_v011_warmup_test.pid already exists; verify whether another futures agent is running')
    ...
    with sqlite3.connect(DB, timeout=30) as conn:
        ...
        try:
            ...
            while True:
                ...
                if args.once:
                    # A normal --once exits immediately. This test variant instead gives
                    # daemon MTF workers bounded time to finish the cold 1D/1H queue,
                    # then runs one real post-warm-up scan before exiting.
                    ...
                    while time.time() < deadline:
                        ...
                        unfinished = getattr(REFRESH_QUEUE, 'unfinished_tasks', queued + pending)
                        now_s = time.time()
                        if now_s - last_report >= 15:
                            emit(type='MTF_WARMUP_PROGRESS', refresh_queue=queued,
                                 refresh_pending=pending, unfinished=unfinished,
                                 seconds_left=max(0, int(deadline - now_s)))
                            last_report = now_s
                        if unfinished == 0 and pending == 0:
                            break
                        time.sleep(1)
                    with REFRESH_LOCK:
                        pending = len(REFRESH_PENDING)
                    emit(type='MTF_WARMUP_DONE', refresh_queue=REFRESH_QUEUE.qsize(),
                         refresh_pending=pending,
                         timed_out=(time.time() >= deadline))
                    emit(type='POST_WARMUP_CYCLE_START', time_utc=now())
                    cycle(conn, args.batch)
                    break
                time.sleep(30)
