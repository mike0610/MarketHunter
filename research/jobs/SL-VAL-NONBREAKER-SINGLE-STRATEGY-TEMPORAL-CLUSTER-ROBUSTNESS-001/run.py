...
OBJECT_ID = 'SL-VAL-NONBREAKER-SINGLE-STRATEGY-TEMPORAL-CLUSTER-ROBUSTNESS-001'
...
def main(out, job_path):
    try:
        ...
        common = {
            ...
            'contract': {
                'question': 'Does UTC close-date clustered dependence materially change the MIXED held-out inference for the same outcome-blind selected non-Breaker strategy?',
                ...
                'dependence_unit': 'UTC calendar date of closed_at within held-out half',
                'minimum_distinct_clusters': MIN_CLUSTERS,
                'bootstrap': {'iterations': BOOTSTRAPS, 'seed': SEED, 'naive': 'trade-level resampling with replacement', 'clustered': 'resample held-out UTC close-date clusters with replacement, retaining all trades in sampled dates', 'interval': '95% percentile CI of held-out mean profit_percent'},
                'terminal_logic': {
                    'TEMPORAL-ROBUST-MIXED': 'naive CI and UTC-date clustered CI both span/touch zero',
                    'TEMPORAL-DEPENDENCE-SENSITIVE': 'naive CI spans/touches zero but UTC-date clustered CI is wholly on one side of zero and is not opposite discovery sign',
                    'TEMPORAL-DEPENDENCE-ADVERSE': 'UTC-date clustered CI is wholly opposite the discovery-half mean sign',
                    'BLOCKED-EVIDENCE': 'selection gates fail, fewer than four held-out UTC-date clusters exist, prior naive inference is not MIXED on the frozen reconstruction, or execution/data failure occurs'
                },
                'interpretation': 'Temporal dependence robustness only. No parameter tuning, strategy promotion/rejection, sizing, allocation, leverage, capital rule, canonical promotion, or causal regime claim.'
            },
        ...
        for r in second:
            groups[utc_date(r['closed_at'])].append(r['profit_percent'])
        cluster_counts = {k: len(v) for k, v in sorted(groups.items())}
        ...
        if len(groups) < MIN_CLUSTERS:
            emit(out, 'BLOCKED-EVIDENCE', reason='held-out half has fewer than four distinct UTC close-date clusters for dependence-aware inference', selected_strategy=selected, selected_counts=rec, distinct_clusters=len(groups), cluster_counts=cluster_counts, max_cluster_share=max_cluster_share, cluster_hhi=hhi, **common)
            return
        ...
        if naive_state != 'mixed':
            ...
            reason = 'frozen reconstruction did not reproduce the prior MIXED naive held-out inference; temporal dependence attribution is not interpretable'
        elif discovery_sign != 'zero' and cluster_state != 'mixed' and cluster_state != discovery_sign:
            state = 'TEMPORAL-DEPENDENCE-ADVERSE'
            reason = 'UTC-date dependence-aware interval resolves wholly opposite the discovery-half sign'
        elif cluster_state != 'mixed':
            state = 'TEMPORAL-DEPENDENCE-SENSITIVE'
            reason = 'UTC-date clustered interval changes the zero-crossing conclusion relative to the naive MIXED interval'
        else:
            state = 'TEMPORAL-ROBUST-MIXED'
            reason = 'held-out uncertainty remains MIXED under both trade-level and UTC close-date clustered resampling'
