...
OBJECT_ID='SL-VAL-TAKER-FLOW-RESIDUAL-INDEX-REPAIR-EXEC-001'
...
EXPECTED_STATES=['OUTCOME-COMPLETE','BLOCKED-EVIDENCE','PROVIDER-BLOCKED']

def emit(out,state,**x):
 p=Path(out);p.mkdir(parents=True,exist_ok=True);(p/'terminal_result.json').write_text(json.dumps({'object_id':OBJECT_ID,'terminal_state':state,**x},indent=2,sort_keys=True),encoding='utf-8')
...
def price_ret(curr,prev,bad,sym,ts,kind):
 if not finite_pos(prev):note_bad(bad,sym,ts,kind+'_denominator',prev);return None
 if not finite_pos(curr):note_bad(bad,sym,ts,kind+'_numerator',curr);return None
 return curr/prev-1.0
def bad_summary(bad):
 vals=sorted(bad.values(),key=lambda x:(x['symbol'],x['ts'],x['kind']));by={s:sum(x['symbol']==s for x in vals) for s in ASSETS};material=len(vals)>MAX_INVALID_TOTAL or any(n>MAX_INVALID_PER_SYMBOL for n in by.values())
...
def month(sym,y,m,bad):
 ...
 with zipfile.ZipFile(io.BytesIO(z)) as q:
  ...
  if len(names)!=1:raise ValueError('archive members '+n)
  for r in csv.reader(io.TextIOWrapper(q.open(names[0]),encoding='utf-8')):
   ...
   flow=(2.0*tb-v)/v;bar_ret=price_ret(c,o,bad,sym,ts,'same_bar_return')
   rows.append((ts,{'open':o,'close':c,'bar_ret':bar_ret,'flow':flow}))
...
def lin_resid(xs,ys,x,y):
 n=len(xs)
 if n<20:return None
 mx=sum(xs)/n;my=sum(ys)/n;vx=sum((z-mx)**2 for z in xs);b=sum((xs[i]-mx)*(ys[i]-my) for i in range(n))/vx if vx>0 else 0.0;a=my-b*mx
 return y-(a+b*x)
def stats(arr):
...
def verdict(s10,s20):
 ...
 return 'REJECTED'
def self_test():
 bad={};a={'open':100.0,'close':100.0,'bar_ret':0.0,'flow':0.1};b={'open':99.0,'close':99.0,'bar_ret':0.0,'flow':-0.1};r=price_ret(b['close'],a['close'],bad,'TEST',1,'close_to_close_return')
 assert abs(r-(-.01))<1e-12 and len(bad)==0
 assert price_ret(101.0,100.0,bad,'TEST',2,'close_to_close_return')>0
 assert price_ret(2.0,0.0,bad,'TEST',3,'fixture') is None

def main(out,job):
 try:self_test();j=json.loads(Path(job).read_text(encoding='utf-8'))
 except Exception as e:emit(out,'BLOCKED-EVIDENCE',reason='self-test/job-read '+repr(e),parameter_tuning=False);return
 if j.get('object_id')!=OBJECT_ID or j.get('executor')!='vps' or j.get('terminal_states')!=EXPECTED_STATES or not(1<=int(j.get('timeout_minutes',0))<=20):emit(out,'BLOCKED-EVIDENCE',reason='job contract mismatch',parameter_tuning=False);return
 data={};files=[];bad={}
 try:
  for sym in ASSETS:
   rows=[]
   ...
   data[sym]=dict(rows)
 except Exception as e:emit(out,'PROVIDER-BLOCKED',reason=repr(e),invalid_price=bad_summary(bad),parameter_tuning=False);return
 if bad_summary(bad)['material']:emit(out,'PROVIDER-BLOCKED',reason='material invalid-price evidence before scoring',invalid_price=bad_summary(bad),source_files=files,parameter_tuning=False);return
 common=sorted(set.intersection(*[set(data[s]) for s in ASSETS]));idx={t:i for i,t in enumerate(common)};residual={s:{} for s in ASSETS}
 for s in ASSETS:
  for i in range(ROLL,len(common)):
   ...
   for h in hist:
    jh=idx[h]
    if jh<=0:continue
    rr=price_ret(data[s][h]['close'],data[s][common[jh-1]]['close'],bad,s,h,'close_to_close_return');y=data[s][h]['flow']
    if rr is None or y is None or not math.isfinite(y):continue
    xs.append(rr);ys.append(y)
   if len(xs)<20 or i==0:continue
   x=price_ret(data[s][t]['close'],data[s][common[i-1]]['close'],bad,s,t,'close_to_close_return');y=data[s][t]['flow']
   if x is None or not math.isfinite(y):continue
   z=lin_resid(xs,ys,x,y)
   if z is not None and math.isfinite(z):residual[s][t]=z
 if bad_summary(bad)['material']:emit(out,'PROVIDER-BLOCKED',reason='material invalid-price evidence during residualization',invalid_price=bad_summary(bad),source_files=files,parameter_tuning=False);return
 ...
 for i,t in enumerate(common):
  ...
  for s in ASSETS:
   ...
   gross=price_ret(data[s][common[i+HOLD+1]]['open'],data[s][common[i+1]]['open'],bad,s,t,'event_open_to_open_return')
   if gross is None:continue
   gross=side*gross;ts_events.append({'ts':t,'symbol':s,'side':side,'residual':z,'net10':gross-COST10,'net20':gross-COST20,'period':period});last_ts[s]=i
  avail=[(residual[s].get(t),s) for s in ASSETS if residual[s].get(t) is not None]
  if len(avail)==len(ASSETS) and i-last_x>=HOLD:
   q=sorted(avail);shorts=[x[1] for x in q[:2]];longs=[x[1] for x in q[-2:]];lr=[];sr=[]
   for s in longs:
    r=price_ret(data[s][common[i+HOLD+1]]['open'],data[s][common[i+1]]['open'],bad,s,t,'xs_open_to_open_return')
    if r is None:break
    lr.append(r)
   for s in shorts:
    r=price_ret(data[s][common[i+HOLD+1]]['open'],data[s][common[i+1]]['open'],bad,s,t,'xs_open_to_open_return')
    if r is None:break
    sr.append(r)
   if len(lr)==2 and len(sr)==2:
    gross=.5*(sum(lr)/2.0)+.5*(sum(-r for r in sr)/2.0);xs_events.append({'ts':t,'longs':longs,'shorts':shorts,'net10':gross-COST10,'net20':gross-COST20,'period':period});last_x=i
 if bad_summary(bad)['material']:emit(out,'PROVIDER-BLOCKED',reason='material invalid-price evidence during event construction',invalid_price=bad_summary(bad),source_files=files,parameter_tuning=False);return
 def pack(ev):
