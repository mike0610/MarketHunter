import io,json,zipfile,urllib.request,urllib.parse
from pathlib import Path
import numpy as np,pandas as pd
OBJ='SL-VAL-FUNDING-EXTREME-MR-001';SYM='BTCUSDT';START=pd.Timestamp('2022-01-01',tz='UTC');END=pd.Timestamp('2026-08-01',tz='UTC');SPLIT=pd.Timestamp('2025-01-01',tz='UTC');OUT=Path('research_output');OUT.mkdir(exist_ok=True)
...
# hourly perpetual klines from monthly archives
def month(y,m):
...
# frozen thresholds based strictly prior 180d of funding observations (~540 obs)
...
tr=[];na=pd.Timedelta(0);last=pd.Timestamp('1970-01-01',tz='UTC')
