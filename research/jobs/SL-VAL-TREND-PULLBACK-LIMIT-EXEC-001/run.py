...
OBJECT_ID='SL-VAL-TREND-PULLBACK-LIMIT-EXEC-001'
...
def main(out,job):
 ...
 for i in range(max(TREND,PULL,ROLL)+1,len(rows)-HOLD):
  ...
  limit_price=rows[i]['c']
  valid_until=min(i+1+HOLD,len(rows)-1)
  fill_i=None
  for k in range(i+1,valid_until+1):
   if rows[k]['l']<=limit_price<=rows[k]['h']:
    fill_i=k;break
  period='IS' if b['ts']<int(SPLIT.timestamp()) else 'OOS'
  if fill_i is None:
   events.append({'ts':b['ts'],'side':side,'trend_ret':trend_ret,'pull_ret':pull_ret,'threshold':th,'filled':False,'period':period})
   last=i
   continue
  exit_i=min(fill_i+HOLD,len(rows)-1)
  exitp=rows[exit_i]['o']
  gross=exitp/limit_price-1 if side=='LONG' else limit_price/exitp-1
  events.append({'ts':b['ts'],'side':side,'trend_ret':trend_ret,'pull_ret':pull_ret,'threshold':th,'filled':True,'fill_i':fill_i,'limit_price':limit_price,'gross':gross,'net':gross-COST,'net20':gross-.002,'period':period})

  last=i
 oall=[x for x in events if x['period']=='OOS'];o=[x for x in oall if x.get('filled')];r=[x['net'] for x in o];r20=[x['net20'] for x in o];s=st(r);s20=st(r20)
 ...
 emit(out,'OUTCOME-COMPLETE',
      contract={'market':'BTCUSDT Spot','tf':'4h','trend':'strictly-prior 180-bar return sign','pullback':'3-bar counter-trend move whose absolute return exceeds strictly-prior 1080-bar 90th percentile','entry':'LIMIT at signal-bar close','validity':'next 12 x 4h bars','fill':'bar-touch; no chase; no market fallback','no_fill':'NO FILL / MISSED OPPORTUNITY','hold_bars_after_fill':12,'decluster_bars':18,'cost':.001,'cost_stress':.002,'split':'2025-01-01','no_sl_tp':True,'parameter_tuning':False},
      is_stats=st([x['net'] for x in events if x['period']=='IS']),
      oos_stats_10bps=s,oos_stats_20bps=s20,oos_signal_count=len(oall),oos_fill_count=len(o),oos_no_fill_count=len(oall)-len(o),oos_fill_rate=(len(o)/len(oall) if oall else None),
