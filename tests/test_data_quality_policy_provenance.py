"""
MarketHunter

Tests for Data Quality Policy Provenance Foundation - Slice 1
(data_quality/policy_provenance.py).
"""

from __future__ import annotations

import ast
import dataclasses
import unittest
from datetime import datetime, timezone
from pathlib import Path

from data_quality.policy_provenance import (
    DataQualityPolicyDeclaration,
    DataQualityPolicyIdentity,
    DataQualityPolicyInvariantError,
    DataQualityPolicyProvenanceDisposition,
    DataQualityPolicyProvenanceError,
    DataQualityPolicyProvenanceRecord,
    DataQualityPolicyProvenanceResult,
    DataQualityPolicyReference,
)

AWARE_NOW = datetime(2026, 8, 21, 12, 0, tzinfo=timezone.utc)
NAIVE_NOW = datetime(2026, 8, 21, 12, 0)


def make_identity(**overrides) -> DataQualityPolicyIdentity:
    kwargs = dict(policy_id="policy-1")
    kwargs.update(overrides)
    return DataQualityPolicyIdentity(**kwargs)


def make_declaration(**overrides) -> DataQualityPolicyDeclaration:
    kwargs = dict(identity=make_identity(), opaque_version="v1")
    kwargs.update(overrides)
    return DataQualityPolicyDeclaration(**kwargs)


def make_reference(**overrides) -> DataQualityPolicyReference:
    kwargs = dict(declaration=make_declaration())
    kwargs.update(overrides)
    return DataQualityPolicyReference(**kwargs)


def make_record(**overrides) -> DataQualityPolicyProvenanceRecord:
    kwargs = dict(policy_reference=make_reference(), available_at=AWARE_NOW)
    kwargs.update(overrides)
    return DataQualityPolicyProvenanceRecord(**kwargs)


class ErrorTaxonomyTests(unittest.TestCase):
    def test_error_hierarchy(self) -> None:
        self.assertTrue(
            issubclass(
                DataQualityPolicyInvariantError,
                DataQualityPolicyProvenanceError,
            )
        )
        self.assertTrue(
            issubclass(DataQualityPolicyProvenanceError, Exception)
        )


class DataQualityPolicyIdentityTests(unittest.TestCase):
    def test_frozen(self) -> None:
        identity = make_identity()
        with self.assertRaises(dataclasses.FrozenInstanceError):
            identity.policy_id = "other"  # type: ignore[misc]

    def test_value_preserved_exactly_with_surrounding_whitespace(self) -> None:
        identity = make_identity(policy_id="  policy-xyz  ")
        self.assertEqual(identity.policy_id, "  policy-xyz  ")

    def test_no_normalization_or_case_folding(self) -> None:
        identity = make_identity(policy_id="Policy-ID-Mixed")
        self.assertEqual(identity.policy_id, "Policy-ID-Mixed")

    def test_blank_policy_id_rejected(self) -> None:
        with self.assertRaises(ValueError):
            make_identity(policy_id="   ")

    def test_wrong_type_rejected(self) -> None:
        with self.assertRaises(TypeError):
            make_identity(policy_id=123)  # type: ignore[arg-type]


class DataQualityPolicyDeclarationTests(unittest.TestCase):
    def test_frozen(self) -> None:
        declaration = make_declaration()
        with self.assertRaises(dataclasses.FrozenInstanceError):
            declaration.opaque_version = "other"  # type: ignore[misc]

    def test_opaque_version_preserved_exactly(self) -> None:
        declaration = make_declaration(opaque_version="  2024-rev-3  ")
        self.assertEqual(declaration.opaque_version, "  2024-rev-3  ")

    def test_opaque_version_never_ordered(self) -> None:
        v2 = make_declaration(opaque_version="v2")
        v10 = make_declaration(opaque_version="v10")
        self.assertNotEqual(v2, v10)
        self.assertEqual(v2.opaque_version, "v2")
        self.assertEqual(v10.opaque_version, "v10")

    def test_blank_opaque_version_rejected(self) -> None:
        with self.assertRaises(ValueError):
            make_declaration(opaque_version="")

    def test_wrong_identity_type_rejected(self) -> None:
        with self.assertRaises(TypeError):
            make_declaration(identity="not-an-identity")  # type: ignore[arg-type]

    def test_construction_is_not_governed_issuance(self) -> None:
        declaration_a = make_declaration()
        declaration_b = make_declaration()
        self.assertIsNot(declaration_a, declaration_b)
        self.assertEqual(declaration_a, declaration_b)


class DataQualityPolicyReferenceTests(unittest.TestCase):
    def test_frozen(self) -> None:
        reference = make_reference()
        with self.assertRaises(dataclasses.FrozenInstanceError):
            reference.declaration = make_declaration()  # type: ignore[misc]

    def test_declaration_preserved_by_identity(self) -> None:
        declaration = make_declaration()
        reference = make_reference(declaration=declaration)
        self.assertIs(reference.declaration, declaration)

    def test_identity_property_delegates_exactly(self) -> None:
        declaration = make_declaration()
        reference = make_reference(declaration=declaration)
        self.assertIs(reference.identity, declaration.identity)

    def test_opaque_version_property_delegates_exactly(self) -> None:
        declaration = make_declaration()
        reference = make_reference(declaration=declaration)
        self.assertEqual(
            reference.opaque_version, declaration.opaque_version
        )

    def test_wrong_declaration_type_rejected(self) -> None:
        with self.assertRaises(TypeError):
            make_reference(declaration="not-a-declaration")  # type: ignore[arg-type]


class DataQualityPolicyProvenanceRecordTests(unittest.TestCase):
    def test_frozen(self) -> None:
        record = make_record()
        with self.assertRaises(dataclasses.FrozenInstanceError):
            record.available_at = AWARE_NOW  # type: ignore[misc]

    def test_policy_reference_preserved_by_identity(self) -> None:
        reference = make_reference()
        record = make_record(policy_reference=reference)
        self.assertIs(record.policy_reference, reference)

    def test_wrong_policy_reference_type_rejected(self) -> None:
        with self.assertRaises(TypeError):
            make_record(policy_reference="not-a-reference")  # type: ignore[arg-type]

    def test_available_at_required_to_be_datetime(self) -> None:
        with self.assertRaises(TypeError):
            make_record(available_at="2026-08-21T12:00:00Z")  # type: ignore[arg-type]

    def test_available_at_must_be_timezone_aware(self) -> None:
        with self.assertRaises(ValueError):
            make_record(available_at=NAIVE_NOW)

    def test_available_at_preserved_exactly(self) -> None:
        record = make_record(available_at=AWARE_NOW)
        self.assertEqual(record.available_at, AWARE_NOW)

    def test_record_does_not_expose_assessment_fields(self) -> None:
        record = make_record()
        field_names = {f.name for f in dataclasses.fields(record)}
        for forbidden in (
            "assessed_at",
            "admissibility",
            "evidence",
            "source",
            "criteria",
            "score",
            "reason",
            "passed",
            "decision",
        ):
            self.assertNotIn(forbidden, field_names)


class DataQualityPolicyProvenanceDispositionTests(unittest.TestCase):
    def test_exact_four_members_no_stale(self) -> None:
        self.assertEqual(
            {member.value for member in DataQualityPolicyProvenanceDisposition},
            {"KNOWN", "UNKNOWN", "UNAVAILABLE", "CONFLICT"},
        )
        self.assertFalse(
            hasattr(DataQualityPolicyProvenanceDisposition, "STALE")
        )


class DataQualityPolicyProvenanceResultTests(unittest.TestCase):
    def test_frozen(self) -> None:
        result = DataQualityPolicyProvenanceResult(
            disposition=DataQualityPolicyProvenanceDisposition.KNOWN,
            record=make_record(),
        )
        with self.assertRaises(dataclasses.FrozenInstanceError):
            result.record = None  # type: ignore[misc]

    def test_known_requires_exactly_one_record(self) -> None:
        record = make_record()
        result = DataQualityPolicyProvenanceResult(
            disposition=DataQualityPolicyProvenanceDisposition.KNOWN,
            record=record,
        )
        self.assertIs(result.record, record)

    def test_known_with_none_record_rejected(self) -> None:
        with self.assertRaises(DataQualityPolicyInvariantError):
            DataQualityPolicyProvenanceResult(
                disposition=DataQualityPolicyProvenanceDisposition.KNOWN,
                record=None,
            )

    def test_unknown_requires_none_record(self) -> None:
        result = DataQualityPolicyProvenanceResult(
            disposition=DataQualityPolicyProvenanceDisposition.UNKNOWN,
            record=None,
        )
        self.assertIsNone(result.record)

    def test_unknown_with_record_rejected(self) -> None:
        with self.assertRaises(DataQualityPolicyInvariantError):
            DataQualityPolicyProvenanceResult(
                disposition=DataQualityPolicyProvenanceDisposition.UNKNOWN,
                record=make_record(),
            )

    def test_unavailable_with_record_rejected(self) -> None:
        with self.assertRaises(DataQualityPolicyInvariantError):
            DataQualityPolicyProvenanceResult(
                disposition=DataQualityPolicyProvenanceDisposition.UNAVAILABLE,
                record=make_record(),
            )

    def test_conflict_requires_none_record(self) -> None:
        result = DataQualityPolicyProvenanceResult(
            disposition=DataQualityPolicyProvenanceDisposition.CONFLICT,
            record=None,
        )
        self.assertIsNone(result.record)

    def test_conflict_with_record_rejected(self) -> None:
        with self.assertRaises(DataQualityPolicyInvariantError):
            DataQualityPolicyProvenanceResult(
                disposition=DataQualityPolicyProvenanceDisposition.CONFLICT,
                record=make_record(),
            )

    def test_wrong_disposition_type_rejected(self) -> None:
        with self.assertRaises(TypeError):
            DataQualityPolicyProvenanceResult(
                disposition="KNOWN", record=make_record()  # type: ignore[arg-type]
            )

    def test_wrong_record_type_rejected(self) -> None:
        with self.assertRaises(TypeError):
            DataQualityPolicyProvenanceResult(
                disposition=DataQualityPolicyProvenanceDisposition.KNOWN,
                record="not-a-record",  # type: ignore[arg-type]
            )


class ScopeDisciplineTests(unittest.TestCase):
    def _module_tree(self):
        import data_quality.policy_provenance as module

        return ast.parse(Path(module.__file__).read_text(encoding="utf-8"))

    def _imported_names(self) -> set[str]:
        imported: set[str] = set()
        for node in ast.walk(self._module_tree()):
            if isinstance(node, ast.ImportFrom):
                if node.module:
                    imported.add(node.module)
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    imported.add(alias.name)
        return imported

    def _referenced_names(self) -> set[str]:
        tree = self._module_tree()
        return {
            node.id for node in ast.walk(tree) if isinstance(node, ast.Name)
        } | {
            node.attr for node in ast.walk(tree) if isinstance(node, ast.Attribute)
        }

    def test_module_is_stdlib_only(self) -> None:
        imported = self._imported_names()
        allowed = {"__future__", "dataclasses", "datetime", "enum"}
        for name in imported:
            self.assertIn(name, allowed, f"unexpected import: {name}")

    def test_no_cross_domain_imports(self) -> None:
        imported = self._imported_names()
        for forbidden in (
            "market_data",
            "exchange",
            "services",
            "models",
            "research",
            "portfolio",
            "portfolio_v1",
            "risk",
            "trade_orchestration",
            "execution",
            "explainability",
            "audit_read_model",
            "manual_review",
            "simulation",
            "time_semantics",
            "api",
            "dashboard",
            "pipeline",
            "strategies",
            "trend_context",
        ):
            self.assertNotIn(forbidden, imported)
            for name in imported:
                self.assertFalse(
                    name.startswith(forbidden + "."),
                    f"unexpected cross-domain import: {name}",
                )

    def test_no_source_domain_object_references(self) -> None:
        referenced = self._referenced_names()
        for forbidden in (
            "ResearchTrade",
            "BaseStrategy",
            "Scanner",
            "Signal",
            "MarketSnapshot",
            "MarketSymbol",
            "CandidateProvenance",
            "SimulationEvent",
            "TrendContextRecord",
            "MarketDataSourceReference",
            "MarketDataObservationRef",
        ):
            self.assertNotIn(forbidden, referenced)

    def test_no_wall_clock_random_db_filesystem_network(self) -> None:
        referenced = self._referenced_names()
        for forbidden in ("now", "utcnow", "uuid4", "today", "time"):
            self.assertNotIn(forbidden, referenced)

        imported = self._imported_names()
        for forbidden in (
            "sqlite3",
            "os",
            "pathlib",
            "subprocess",
            "requests",
            "fastapi",
            "httpx",
            "socket",
            "random",
        ):
            self.assertNotIn(forbidden, imported)

    def test_no_current_latest_nearest_selector_exported(self) -> None:
        import data_quality.policy_provenance as module

        for forbidden in (
            "current",
            "latest",
            "nearest",
            "default",
            "get_current",
            "get_latest",
            "winner",
        ):
            self.assertFalse(hasattr(module, forbidden))

    def test_no_sort_or_min_max_calls(self) -> None:
        for node in ast.walk(self._module_tree()):
            if (
                isinstance(node, ast.Call)
                and isinstance(node.func, ast.Name)
                and node.func.id in ("sorted", "min", "max")
            ):
                self.fail(f"unexpected {node.func.id}() call in module")

    def test_no_manifest_history_writer_issuer_registry_surface(self) -> None:
        import data_quality.policy_provenance as module

        module_names = {name for name in dir(module) if not name.startswith("_")}
        for forbidden in (
            "Manifest",
            "History",
            "Repository",
            "Issuer",
            "Writer",
            "Registry",
            "Assessment",
            "Decision",
            "Admissibility",
        ):
            for name in module_names:
                self.assertNotIn(forbidden, name)


if __name__ == "__main__":
    unittest.main()
