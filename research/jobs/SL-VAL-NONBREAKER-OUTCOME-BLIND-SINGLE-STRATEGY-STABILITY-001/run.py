...
OBJECT_ID = 'SL-VAL-NONBREAKER-OUTCOME-BLIND-SINGLE-STRATEGY-STABILITY-001'
...
def main(out, job_path):
    try:
        ...
        common = {
            ...
            'contract': {
                'question': 'Does the discovery-half return sign of the most represented eligible non-Breaker strategy, selected strictly by trade count, persist in the held-out chronological half?',
                'selection': 'Outcome-blind. Among strategies passing the frozen non-outcome evidence gates, select maximum eligible_timestamped trade count; lexical strategy name is the deterministic tie-breaker. Breaker is excluded because of the independently established entry-zone validity confound.',
                ...
                'terminal_logic': {
                    ...
                    'BLOCKED-EVIDENCE': 'no strategy passes frozen non-outcome evidence gates or execution/data failure occurs',
                },
        ...
        if not candidate_data:
            emit(
                ...
                reason='no non-Breaker strategy met frozen count/half/timestamp-coverage gates',
                **common
            )
            return
        ...
        (ep / 'single_strategy_stability_evidence.json').write_text(raw)
        ev_sha = hashlib.sha256(raw.encode()).hexdigest()
        emit(
            out, state,
            reason=reason,
            evidence_file='single_strategy_stability_evidence.json',
