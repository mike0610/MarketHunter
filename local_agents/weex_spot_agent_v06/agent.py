...
def fetch_raw(path, params=None, attempts=FAST_ATTEMPTS):
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'WEEX-spot-radar/0.15-controlled-m15-paper'})
    last_exc = None
...
def analyze(conn, symbol, m15=None):
    ...
    return {'symbol': symbol, 'state': state, 'direction': direction,
            ...
            'rules_version': 'experimental_v0_15_controlled_m15_long_short_1d_1h_15m'}
...
def _m15_prefetch_one(symbol):
    started = time.monotonic()
    try:
        raw = fetch_raw('/api/v3/market/klines', {'symbol': symbol, 'interval': '15m', 'limit': 100})
        current = int(time.time() * 1000)
        rows = []
        if not isinstance(raw, list):
